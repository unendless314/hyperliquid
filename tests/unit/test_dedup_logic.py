...
from hyperliquid.storage.db import event_key, get_system_state, init_db
...
def test_ingest_dedup_updates_cursor() -> None:
    with tempfile.NamedTemporaryFile(suffix=".db") as tmp:
        ...
        events = ingest.ingest_raw_events([raw, raw], conn)

        assert len(events) == 1
        assert _count_processed(conn) == 1
        assert get_system_state(conn, "last_processed_timestamp_ms") == str(
            raw.timestamp_ms
        )
        assert get_system_state(conn, "last_processed_event_key") == event_key(
